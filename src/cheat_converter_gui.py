...
def create_eden_files(build_id, cheats):
...
def main():
    ...
    def convert_now():
        ...
        if not cheats:
            ...
            return
        created = create_eden_files(build_id, cheats)
        messagebox.showinfo("Success", f"Created {len(created)} cheat files under:\n{build_id}/")
    ...
    if dnd_available and "TkinterDnD" in globals():
        ...
        window = tk.Tk()
    window.title("Cheat File Splitter")
    ...
    # Convert button
    convert_button = tk.Button(window, text="Split Cheat File Into Folders", command=convert_now, height=2)
